...
static LANGUAGE_CONFIGS: Lazy<HashMap<&'static str, LanguageConfig>> = Lazy::new(|| {
    ...
    configs.insert("rs", LanguageConfig {
    ...
    configs.insert("py", LanguageConfig {
    ...
    configs.insert("js", LanguageConfig {
    ...
    configs.insert("ts", LanguageConfig {
...
impl FileParser {
    pub fn new() -> Result<Self> {
        ...
        Ok(Self { parsers: std::sync::Mutex::new(parsers) })
    }
    ...
    pub fn parse_file(&self, path: &Path, content: &str) -> Result<Vec<AiCall>> {
        ...
        let config = LANGUAGE_CONFIGS.get(ext)
